...
def parse_arguments():
    ...
    parser.add_argument('--api_key_qw', type=str, required=True, help='API 密钥 qwen-vl')
    parser.add_argument('--api_key', type=str, required=True, help='API 密钥 GPT')
...
def encode_image(image_path):
        ...
        return r"file://{}".format(os.path.abspath(image_path))


def get_r(messages,api_key,model='qwen-vl-plus',temperature=0.5):
    print(messages)
    ...
    resText = response.output.choices[0]['message']['content'][0]['text']
    print(resText)
    return resText
...
if __name__ == "__main__":
    args = parse_arguments()
    print(f"API Key QWVL: {args.api_key_qw}")   
